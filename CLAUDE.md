...
- Practice restart: `PracticePage` owns `activeQueue` + `restartKey`; "Practice Again" reshuffles and bumps `restartKey`, which is passed as `key={restartKey}` to `SessionView` to force a clean remount. Do NOT render a nested `<SessionView>` inside `SuccessScreen` — it leaves the outer instance mounted, duplicating window keydown listeners + audioRefs and causing audio overlap on arrow keys.
